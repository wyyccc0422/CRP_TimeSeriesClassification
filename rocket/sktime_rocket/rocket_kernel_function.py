# -*- coding: utf-8 -*-
"""
Created on Fri Feb  2 14:32:20 2024
...
#fucntion to generate kernels
@njit(
    "Tuple((float32[:],int32[:],float32[:],int32[:],int32[:],int32[:],"
    "int32[:]))(int32,int32,int32,optional(int32))",
    cache=True,
)
def _generate_kernels(n_timepoints, num_kernels, n_columns, seed = None):
    '''
    n_timepoints -> length of time series input X [int]
    num_kernels -> number of random kernels [int]
    n_columns -> dimension of time series input X [int] /  in our case: 1, only GR
    seed -> random seed
    '''
    if seed is not None:
        np.random.seed(seed)
    ...
    lengths = np.random.choice(candidate_lengths, num_kernels).astype(np.int32)   #generate random length for all kernels
    #check different candidate lengths and shape
    ...
    #channel_indices = np.zeros(num_kernels, dtype = np.int32)
...
#function to apply kernels on univariate data
...
#function to apply kernels on multivariate 
@njit(fastmath=True, cache=True)
